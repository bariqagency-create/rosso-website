...
// Canonical job statuses shown in the UI.
export const STATUSES = ['new', 'in_progress', 'waiting_parts', 'ready', 'delivered', 'cancelled'];
// Legacy values tolerated when reading old rows.
export const LEGACY_STATUSES = ['pending', 'confirmed', 'completed'];
export const ALL_STATUSES = [...STATUSES, ...LEGACY_STATUSES];
...
// Map legacy DB values to canonical ones for display + save.
export function normalizeStatus(s) {
  if (s === 'pending')   return 'new';
  if (s === 'confirmed') return 'new';
  if (s === 'completed') return 'delivered';
  return s || 'new';
}

export const PAYMENT_STATUSES = ['unpaid', 'partial', 'paid'];
export const PAYMENT_METHODS = ['cash', 'instapay', 'card', 'wallet', 'bank_transfer', 'other'];
...
function buildRecord(data, meta = {}) {
  const vehicle = [data.year, data.brand, data.model].filter(Boolean).join(' ').trim();
...
function rowToRecord(row) {
  return {
    ...
    lang: row.lang || 'en',
    data: row.data || {},
    // Financial / workflow fields (may be null for older rows)
...
// Patch any subset of booking financial / workflow fields.
// Accepts camelCase or DB-style keys; maps to columns in `bookings`.
const BOOKING_FIELD_MAP = {
  status: 'status',
  paymentStatus: 'payment_status',
  paymentMethod: 'payment_method',
  laborSellingPrice: 'labor_selling_price',
  discount: 'discount',
...
export async function updateBooking(id, patch) {
  ...
  for (const [k, v] of Object.entries(patch || {})) {
    const col = BOOKING_FIELD_MAP[k] || k;
    row[col] = v;
  }
  row.updated_at = new Date().toISOString();
  if (!supabaseConfigured) return;
  const { error } = await supabase.from(BOOKINGS_TABLE).update(row).eq('id', id);
